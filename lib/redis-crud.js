...
util.inherits(BridgeToRedis, Connector);

BridgeToRedis.prototype.connect = function (callback) {
...
BridgeToRedis.prototype.destroyAll = function destroyAll(model, where, callback) {
  ...
  if(where) {
    console.log(where);
    this.find(model, {where: where}, function(err, results) {
      if(err || !results) {
...
BridgeToRedis.prototype.automigrate = function automigrate(models, cb) {
  var self = this;
...
BridgeToRedis.prototype.ping = function (cb) {
  var self = this;
  if (self.client) {
    this.client.ping(cb);
  } else {
